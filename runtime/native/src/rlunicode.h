...
RLUC_API int ConvertUTF8ToUTF16(const char *text, uint16_t *out); // Convert zero terminated string from UTF8 to UTF16, or calculate buffer needed if out is NULL
RLUC_API int ConvertUTF16ToUTF8(const uint16_t *text, char *out); // Convert zero terminated string from UTF16 to UTF8, or calculate buffer needed if out is NULL
RLUC_API int ConvertUTF8ToUTF32(const char *text, uint32_t *out); // Convert zero terminated string from UTF8 to UTF32, or calculate buffer needed if out is NULL
RLUC_API int ConvertUTF32ToUTF8(const uint32_t *text, char *out); // Convert zero terminated string from UTF32 to UTF8, or calculate buffer needed if out is NULL
RLUC_API int ConvertUTF16ToUTF32(const uint16_t *text, uint32_t *out); // Convert zero terminated string from UTF16 to UTF32, or calculate buffer needed if out is NULL
RLUC_API int ConvertUTF32ToUTF16(const uint32_t *text, uint16_t *out); // Convert zero terminated string from UTF32 to UTF16, or calculate buffer needed if out is NULL
...
int ConvertUTF8ToUTF16(const char *text, uint16_t *out)
{
    int outputLength = 0;
    while(*text)
    {
        int size = 0;
        int codepoint = GetCodepointFromUTF8(text, &size);
        if (codepoint <= 0xffff) {
            ++outputLength;
...
int ConvertUTF16ToUTF8(const uint16_t *text, char *out)
{
    int outputLength = 0;
    const char* utf8;
    while(*text)
    {
        int size = 0;
        int bytes = 0;
        int codepoint = GetCodepointFromUTF16(text, &size);
...
int ConvertUTF32ToUTF8(const uint32_t *text, char *out)
{
    int outputLength = 0;
    const char* utf8;
    while(*text)
    {
        int bytes = 0;
        int codepoint = GetCodepointFromUTF32(text);
        utf8 = CodepointToUTF8(codepoint, &bytes);
        if(out) memcpy(out + outputLength, utf8, bytes);
        outputLength += bytes;
        ++text;
...
int ConvertUTF8ToUTF32(const char *text, uint32_t *out)
{
    int outputLength = 0;
    while(*text)
    {
...
int ConvertUTF16ToUTF32(const uint16_t *text, uint32_t *out)
{
    int outputLength = 0;
    while(*text)
    {
...
int ConvertUTF32ToUTF16(const uint32_t *text, uint16_t *out)
{
    int outputLength = 0;
    while(*text)
    {
